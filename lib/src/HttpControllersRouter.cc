    ...
    const std::vector<internal::HttpConstraint> &constraints)
{
    ...
    binder->handlerAnnotations_ = result.annotations;
    ...
    const std::map<std::string, std::string> &annotations)
{
    ...
    binderInfo->handlerAnnotations_ = annotations;
...
RouteResult HttpControllersRouter::route(const HttpRequestImplPtr &req)
{
    ...
    if (!binder->handlerAnnotations_.empty())
    {
        req->attributes()->insert("drogon::annotations",
                                  binder->handlerAnnotations_);
    }
    return {RouteResult::Success, binder};
}
